export default function Home() {
  return (
    <main>
      ...
      {/* SECCIÓN DARK (quiebre — Control Room iría acá) */}
      <section className="bg-pimenton-dark px-8 py-24">
        <div className="max-w-5xl mx-auto space-y-8">
          <p className="text-pimenton-accent text-xs uppercase tracking-[0.18em]">
            quiebre dark · control room
          </p>
          <h2 className="text-pimenton-text-on-dark text-5xl font-semibold leading-[1.1] tracking-tight">
            Tu delivery operado desde una{" "}
            <span className="text-pimenton-accent">sola pantalla</span>.
          </h2>
          <p className="text-pimenton-text-on-dark-muted text-base max-w-xl">
            Aquí va el control room. Esta sección demuestra el quiebre visual.
          </p>

          <div className="grid grid-cols-4 gap-4 pt-8">
            {[
              { name: "+500", label: "clientes" },
              { name: "+12", label: "países" },
              { name: "~30%", label: "crecimiento" },
              { name: "+6M", label: "pedidos/año" },
            ].map((s) => (
              <div key={s.name} className="border border-pimenton-dark-border rounded-lg p-6">
                <p className="text-pimenton-accent text-3xl font-semibold">{s.name}</p>
                <p className="text-pimenton-text-on-dark-muted text-sm mt-2">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
